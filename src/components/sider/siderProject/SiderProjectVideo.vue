...
<template>
    <div id="project-video" class="px-4">
        <n-form label-placement="left" :label-width="90" label-align="left" size="small">
            <n-form-item :label="t('layout.sider.project.resolution')">
                ...
                <EsInputNumber v-model:value="videoFPS" :min="15" :max="240" size="tiny" :step="1" :show-button="false"
                    :bordered="false" class="w-6 ml-2" />
                <span>fps</span>
            </n-form-item>

            <n-form-item :label="t('layout.sider.project.duration')">
                <EsInputNumber v-model:value="videoDuration" :min="1" size="tiny" :step="1" :show-button="false"
                    :bordered="false" class="w-16" />
                <span>{{ t('layout.sider.project.seconds') }}</span>
            </n-form-item>
        </n-form>

        <div class="w-full text-center">
            <n-progress type="line" status="success" v-if="rendering" :percentage="percentage * 100">{{ (percentage * 100).toFixed(2) }}%
            </n-progress>
            <n-button type="primary" size="small" :loading="rendering" class="w-24" @click="renderVideo">{{
                    t('layout.sider.project.render')
            }}</n-button>
        </div>
    </div>
</template>
